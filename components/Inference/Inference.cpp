 ...
 * @return [std::vector<std::pair<int, int>> valid adjacent rooms]
 */
std::vector<std::pair<int, int>> Inference::get_adjacent_rooms(std::pair<int, int> current_room)
{
  std::vector<std::pair<int, int>> adjacent_rooms;
  std::vector<std::pair<int, int>> adjacent_rooms_candidates;
  adjacent_rooms_candidates.push_back(std::make_pair(current_room.first + 1, current_room.second));
  adjacent_rooms_candidates.push_back(std::make_pair(current_room.first - 1, current_room.second));
  adjacent_rooms_candidates.push_back(std::make_pair(current_room.first, current_room.second + 1));
  adjacent_rooms_candidates.push_back(std::make_pair(current_room.first, current_room.second - 1));
...
DataStructures::Decision Inference::infer(std::pair<int, int> current_room)
{
  ...
  // std::std::vector<deduction, std::set<std::std::pair<int, int>, bool>> model;
  ...
  // test
  // get_adjacent_rooms(make_std::pair(0,0));
  // get_adjacent_rooms(make_std::pair(0,2));
  // get_adjacent_rooms(make_std::pair(3,1));
  // get_adjacent_rooms(make_std::pair(3,3));
  // get_adjacent_rooms(make_std::pair(-1,-1));
  // get_adjacent_rooms(make_std::pair(4,4));
